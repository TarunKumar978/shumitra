...
function CtaForm() {
  ...
  const inputStyle = {
    ...
    border:"1.5px solid rgba(13,27,42,0.12)", borderRadius:"12px",
    background:"white", fontSize:"14px", color:"#0D1B2A",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/quote", {
        ...
        body: JSON.stringify({ ...form, company:"", message:`Spec: ${form.spec} | Port: ${form.port} | Incoterm: ${form.incoterm}` }),
      });
      const data = await res.json();
      if (data.success) setDone(true);
      else setError("Something went wrong. Please WhatsApp us.");
    } catch { setError("Something went wrong. Please WhatsApp us."); }
  ...
  if (done) return (
    <div style={{ textAlign:"center", padding:"24px 0" }}>
      ...
      <a href="https://wa.me/917259829005" target="_blank" rel="noopener noreferrer"
        style={{ display:"inline-flex", alignItems:"center", gap:"8px", background:"#22c55e", color:"white", fontWeight:700, padding:"10px 20px", borderRadius:"10px", textDecoration:"none", fontSize:"13px" }}>
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px" }}>
        ...
        <input type="text" placeholder="Country / Destination" value={form.country}
          onChange={e => setForm(f => ({ ...f, country:e.target.value }))} style={inputStyle} />
      </div>
      <input type="text" placeholder="Product Name (e.g. Turmeric, Basmati Rice...)" value={form.product}
        onChange={e => setForm(f => ({ ...f, product:e.target.value }))} style={inputStyle} required />
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"10px" }}>
        <input type="text" placeholder="Quantity" value={form.quantity}
      ...
      <input type="text" placeholder="Grade / Specification (e.g. Curcumin 3–5%, ASTA 60+...)" value={form.spec}
        onChange={e => setForm(f => ({ ...f, spec:e.target.value }))} style={inputStyle} />
      <input type="text" placeholder="Destination Port (e.g. Jebel Ali, Rotterdam...)" value={form.port}
        onChange={e => setForm(f => ({ ...f, port:e.target.value }))} style={inputStyle} />
      {error && <p style={{ color:"#dc2626", fontSize:"12px", margin:0 }}>⚠️ {error}</p>}
      <button type="submit" disabled={loading}
        style={{ width:"100%", background: loading ? "rgba(13,27,42,0.1)" : "linear-gradient(135deg,#C4930A,#E8A020)", color: loading ? "rgba(13,27,42,0.3)" : "white", fontWeight:700, padding:"14px", borderRadius:"12px", border:"none", cursor: loading ? "not-allowed" : "pointer", fontSize:"15px", boxShadow: loading ? "none" : "0 6px 20px rgba(196,147,10,0.35)", marginTop:"4px" }}>
...
function FeedbackSection() {
  ...
  const inputStyle = {
    width:"100%", padding:"12px 16px",
    border:"1.5px solid rgba(13,27,42,0.1)", borderRadius:"12px",
    background:"white", fontSize:"13px", color:"#0D1B2A",
    outline:"none", boxSizing:"border-box" as const,
    fontFamily:"DM Sans, sans-serif",
  };
  ...
  if (submitted) return (
    <div style={{ textAlign:"center", padding:"48px 24px" }}>
      ...
      <p style={{ color:"rgba(13,27,42,0.55)", fontSize:"15px", lineHeight:1.7 }}>Your feedback means the world to us. We'll use it to keep improving every shipment.</p>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div style={{ textAlign:"center", marginBottom:"28px" }}>
        ...
        <div style={{ display:"flex", justifyContent:"center", gap:"8px", marginBottom:"10px" }}>
          {[1,2,3,4,5].map(star => (
            <button key={star} type="button"
              ...
              <svg width="38" height="38" viewBox="0 0 24 24" fill={(hover || rating) >= star ? "#E8A020" : "none"} stroke={(hover || rating) >= star ? "#E8A020" : "rgba(13,27,42,0.25)"} strokeWidth="1.5">
        ...
        {(hover || rating) > 0 && (
          <div style={{ display:"inline-flex", alignItems:"center", gap:"6px", background:`${colors[hover || rating]}15`, border:`1px solid ${colors[hover || rating]}30`, borderRadius:"50px", padding:"4px 14px" }}>
            <span style={{ color:colors[hover || rating], fontSize:"13px", fontWeight:700 }}>{labels[hover || rating]}</span>
          </div>
        )}
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"10px", marginBottom:"10px" }}>
        <div>
          <label style={{ fontSize:"11px", fontWeight:700, color:"rgba(13,27,42,0.45)", textTransform:"uppercase", letterSpacing:"0.1em", display:"block", marginBottom:"6px" }}>Your Name *</label>
          <input type="text" placeholder="John Smith" value={form.name} onChange={e => setForm(f => ({ ...f, name:e.target.value }))} style={inputStyle} required />
        </div>
        <div>
          <label style={{ fontSize:"11px", fontWeight:700, color:"rgba(13,27,42,0.45)", textTransform:"uppercase", letterSpacing:"0.1em", display:"block", marginBottom:"6px" }}>Company</label>
          <input type="text" placeholder="Acme Trading Co." value={form.company} onChange={e => setForm(f => ({ ...f, company:e.target.value }))} style={inputStyle} />
        </div>
      </div>
      <div style={{ marginBottom:"10px" }}>
        <label style={{ fontSize:"11px", fontWeight:700, color:"rgba(13,27,42,0.45)", textTransform:"uppercase", letterSpacing:"0.1em", display:"block", marginBottom:"6px" }}>Country</label>
        <input type="text" placeholder="UAE, USA, UK..." value={form.country} onChange={e => setForm(f => ({ ...f, country:e.target.value }))} style={inputStyle} />
      </div>
      <div style={{ marginBottom:"20px" }}>
        <label style={{ fontSize:"11px", fontWeight:700, color:"rgba(13,27,42,0.45)", textTransform:"uppercase", letterSpacing:"0.1em", display:"block", marginBottom:"6px" }}>Your Feedback *</label>
        <textarea placeholder="Tell us about your experience — quality, communication, delivery, documentation..." rows={4}
      ...
      <button type="submit" disabled={rating === 0 || loading}
        style={{ width:"100%", background: rating > 0 ? "linear-gradient(135deg,#C4930A,#E8A020)" : "rgba(13,27,42,0.08)", color: rating > 0 ? "white" : "rgba(13,27,42,0.3)", fontWeight:700, padding:"14px", borderRadius:"12px", border:"none", cursor: rating > 0 ? "pointer" : "not-allowed", fontSize:"15px", transition:"all 0.2s", boxShadow: rating > 0 ? "0 6px 20px rgba(196,147,10,0.3)" : "none" }}>
    ...
    <>
      ...
      <section style={{ position:"relative", height:"100vh", minHeight:"640px", overflow:"hidden" }}>
        ...
        <div style={{ position:"absolute", top:0, left:0, right:0, zIndex:10, padding:"96px 64px 0" }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:"8px", border:"1px solid rgba(196,147,10,0.45)", borderRadius:"50px", padding:"7px 18px", marginBottom:"24px", background:"rgba(0,0,0,0.35)", backdropFilter:"blur(12px)" }}>
          ...
          <h1 style={{ fontFamily:"DM Serif Display,Georgia,serif", color:"white", lineHeight:1.0, margin:"16px 0 18px", fontSize:"clamp(44px,6vw,84px)", fontWeight:400, textShadow:"0 4px 40px rgba(0,0,0,0.7)", maxWidth:"720px" }}>
            India's Finest<br />
            <span style={{ background:"linear-gradient(90deg,#F5C842,#E8A020)", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text" }}>Spices &</span><br />
            Commodities
          </h1>
          <p style={{ color:"rgba(255,255,255,0.78)", fontSize:"16px", lineHeight:1.75, marginBottom:"32px", maxWidth:"480px", textShadow:"0 2px 16px rgba(0,0,0,0.5)" }}>
            <em style={{ color:"white", fontFamily:"DM Serif Display,Georgia,serif", fontSize:"18px" }}>"Rooted in India. Made for the World."</em><br />
            Pan-India sourcing · 29 states · 100+ products · 50+ countries
          </p>
          <div style={{ display:"flex", gap:"12px", flexWrap:"wrap" }}>
            <Link href="/products" style={{ display:"inline-flex", alignItems:"center", gap:"8px", background:"linear-gradient(135deg,#E8A020,#C4930A)", color:"#0D1B2A", fontWeight:700, padding:"14px 28px", borderRadius:"14px", textDecoration:"none", fontSize:"15px", boxShadow:"0 8px 32px rgba(196,147,10,0.45)" }}>
              Explore Products <ArrowRight size={16} />
            </Link>
            <button type="button" onClick={() => { setQuoteProduct(""); setQuoteOpen(true); }} style={{ display:"inline-flex", alignItems:"center", gap:"8px", background:"rgba(255,255,255,0.1)", border:"1.5px solid rgba(255,255,255,0.35)", color:"white", fontWeight:600, padding:"14px 24px", borderRadius:"14px", fontSize:"15px", backdropFilter:"blur(12px)", cursor:"pointer" }}>
              Request a Quote
            </button>
            <a href="https://wa.me/917259829005" style={{ display:"inline-flex", alignItems:"center", gap:"7px", background:"rgba(37,211,102,0.12)", border:"1.5px solid rgba(37,211,102,0.4)", color:"#4ADE80", fontWeight:600, padding:"14px 20px", borderRadius:"14px", textDecoration:"none", fontSize:"15px", backdropFilter:"blur(12px)" }}>
              💬 WhatsApp
            </a>
          </div>
        </div>
        ...
        {/* BOTTOM CONTENT */}
        <div style={{ position:"absolute", bottom:0, left:0, right:0, zIndex:10, padding:"0 64px 36px" }}>
          <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", gap:"32px" }}>
            <div style={{ display:"flex", gap:"0", backdropFilter:"blur(14px)", background:"rgba(0,0,0,0.4)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:"16px", overflow:"hidden" }}>
              {stats.map((s, i) => (
                <div key={i} style={{ padding:"14px 24px", borderRight: i < stats.length-1 ? "1px solid rgba(255,255,255,0.07)" : "none", textAlign:"center", minWidth:"90px" }}>
                  <p style={{ fontFamily:"DM Serif Display,Georgia,serif", fontSize:"24px", fontWeight:700, color:"#E8A020", margin:"0 0 2px" }}>
                    <Counter target={parseInt(s.value)} suffix={s.value.includes("+") ? "+" : ""} />
                  </p>
                  <p style={{ color:"rgba(255,255,255,0.38)", fontSize:"8px", margin:0, letterSpacing:"0.08em", textTransform:"uppercase" }}>{s.label}</p>
                </div>
              ))}
            </div>
        ...
        <div style={{ position:"absolute", bottom:"140px", left:"50%", transform:"translateX(-50%)", zIndex:10, color:"rgba(255,255,255,0.18)", animation:"bounce 2s infinite" }}>
          <ChevronDown size={22} />
        </div>
        <style>{`
          @keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.4;transform:scale(1.6)}}
          @keyframes bounce{0%,100%{transform:translateX(-50%) translateY(0)}50%{transform:translateX(-50%) translateY(-8px)}}
          @keyframes floatBig{0%,100%{transform:translateY(-50%) rotate(-5deg)}50%{transform:translateY(-53%) rotate(5deg)}}
      ...
      <div style={{ background:"linear-gradient(90deg,#92400e,#C4930A,#E8A020,#C4930A,#92400e)", padding:"12px 0", overflow:"hidden" }}>
        <div style={{ display:"flex", alignItems:"center", whiteSpace:"nowrap", animation:"marquee 30s linear infinite" }}>
          {[...Array(4)].map((_,i) =>
            ["FSSAI Certified","APEDA Registered","Spices Board Certified","ISO 9001:2015","IEC Licensed","50+ Countries","100+ Products","Pan-India Sourcing","FOB · CIF · DAP"].map(item => (
              <span key={`${i}-${item}`} style={{ color:"#1C0A00", fontWeight:700, fontSize:"11px", padding:"0 26px", display:"inline-flex", alignItems:"center" }}>
                <span style={{ width:"3px", height:"3px", background:"rgba(28,10,0,0.3)", borderRadius:"50%", display:"inline-block", marginRight:"26px" }} />{item}
              </span>
            ))
          )}
        </div>
      ...
      <section style={{ padding:"48px 0 64px", background:"#F5F0E8" }}>
        <div style={{ maxWidth:"1400px", margin:"0 auto", padding:"0 32px" }}>
          <div style={{ textAlign:"center", marginBottom:"40px" }}>
            ...
            <h2 style={{ fontFamily:"DM Serif Display,Georgia,serif", fontSize:"clamp(30px,4vw,48px)", color:"#0D1B2A", margin:"0 0 10px" }}>Our Product Range</h2>
              ...
              return (
                <div key={product.id} style={{ background:"white", borderRadius:"20px", overflow:"hidden", border:"1px solid rgba(13,27,42,0.08)", boxShadow:"0 2px 12px rgba(0,0,0,0.05)", display:"flex", flexDirection:"column" }}>
                  ...
                  <div style={{ padding:"16px", flex:1, display:"flex", flexDirection:"column" }}>
                    <div style={{ marginBottom:"10px" }}>
                      <p style={{ fontFamily:"DM Serif Display,Georgia,serif", fontSize:"17px", color:"#0D1B2A", margin:"0 0 4px", fontWeight:400 }}>{product.name}</p>
                    ...
                    <div style={{ display:"flex", gap:"6px" }}>
                      <Link href={`/products/${product.id}`} style={{ flex:1, display:"inline-flex", alignItems:"center", justifyContent:"center", gap:"4px", background:"#0D1B2A", color:"white", fontWeight:600, padding:"9px 0", borderRadius:"10px", textDecoration:"none", fontSize:"11px" }}>
                        View Specs <ArrowRight size={11} />
                      </Link>
                      <button type="button" onClick={() => { setQuoteProduct(product.id); setQuoteOpen(true); }} style={{ flex:1, display:"inline-flex", alignItems:"center", justifyContent:"center", background:"linear-gradient(135deg,#C4930A,#E8A020)", color:"white", fontWeight:600, padding:"9px 0", borderRadius:"10px", fontSize:"11px", border:"none", cursor:"pointer" }}>
                        Get Quote
                      </button>
                    </div>
                  </div>
          ...
          <div style={{ textAlign:"center", marginTop:"40px" }}>
            <Link href="/products" style={{ display:"inline-flex", alignItems:"center", gap:"8px", background:"#0D1B2A", color:"white", fontWeight:700, padding:"14px 40px", borderRadius:"14px", textDecoration:"none", fontSize:"14px" }}>
      ...
      <section style={{ padding:"48px 0", background:"white" }}>
        <div style={{ maxWidth:"1280px", margin:"0 auto", padding:"0 48px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"16px" }}>
            {[
              { icon:"🌱", title:"Direct Sourcing", desc:"Straight from farms across 29 states. No middlemen, full traceability field-to-port." },
              { icon:"📋", title:"Full Documentation", desc:"COA, phytosanitary, FSSAI, fumigation — all export paperwork handled end-to-end." },
              { icon:"📦", title:"Custom Packaging", desc:"PP bags, jute, vacuum, retail packs. Private label and white-label ready." },
              { icon:"🤝", title:"Personal Service", desc:"Direct access to our team. Real accountability on every single order." },
            ].map((item, i) => (
              <div key={i} style={{ background:"white", border:"1px solid rgba(13,27,42,0.08)", borderRadius:"18px", padding:"24px" }}>
                <div style={{ fontSize:"32px", marginBottom:"14px" }}>{item.icon}</div>
                <h3 style={{ fontFamily:"DM Serif Display,Georgia,serif", fontSize:"17px", color:"#0D1B2A", marginBottom:"8px" }}>{item.title}</h3>
                <p style={{ color:"rgba(13,27,42,0.55)", fontSize:"13px", lineHeight:1.65, margin:0 }}>{item.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ position:"relative", overflow:"hidden" }}>
          <div style={{ display:"flex", gap:"24px", animation:"certScroll 18s linear infinite", width:"max-content" }}>
            {[
              ...
              <div key={i} style={{ display:"flex", alignItems:"center", gap:"16px", background:"#F5F0E8", border:"1px solid rgba(13,27,42,0.07)", borderRadius:"18px", padding:"20px 28px", flexShrink:0, minWidth:"320px" }}>
      ...
      <section style={{ padding:"48px 0", background:"#F5F0E8" }}>
        <div style={{ maxWidth:"1280px", margin:"0 auto", padding:"0 48px" }}>
          <div style={{ textAlign:"center", marginBottom:"36px" }}>
            ...
            <h2 style={{ fontFamily:"DM Serif Display,Georgia,serif", fontSize:"clamp(26px,3.5vw,42px)", color:"#0D1B2A", margin:0 }}>From Enquiry to Delivery</h2>
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:"16px" }}>
            {[
              { n:"01", t:"Inquiry", d:"Share product, quantity & destination" },
              { n:"02", t:"Quote", d:"Pricing & specs within 24 hrs" },
              { n:"03", t:"Sample", d:"Physical sample dispatched" },
              { n:"04", t:"Order", d:"PO confirmed, QC begins" },
              { n:"05", t:"Delivery", d:"FOB/CIF with full documentation" },
            ].map(s => (
              <div key={s.n} style={{ display:"flex", flexDirection:"column", alignItems:"center", textAlign:"center" }}>
                <div style={{ width:"54px", height:"54px", borderRadius:"14px", background:"#0D1B2A", color:"#E8A020", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"monospace", fontWeight:700, fontSize:"14px", marginBottom:"14px", boxShadow:"0 6px 18px rgba(13,27,42,0.15)" }}>{s.n}</div>
                <h4 style={{ fontFamily:"DM Serif Display,Georgia,serif", fontSize:"16px", color:"#0D1B2A", marginBottom:"6px" }}>{s.t}</h4>
                <p style={{ color:"rgba(13,27,42,0.45)", fontSize:"12px", lineHeight:1.55, margin:0 }}>{s.d}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ padding:"48px 0", background:"#F5F0E8" }}>
        <div style={{ maxWidth:"1280px", margin:"0 auto", padding:"0 48px" }}>
          <div style={{ textAlign:"center", marginBottom:"28px" }}>
            ...
            <h2 style={{ fontFamily:"DM Serif Display,Georgia,serif", fontSize:"clamp(32px,4vw,52px)", color:"#0D1B2A", margin:"0 0 12px" }}>
              Rooted in India. <span style={{ color:"#C4930A" }}>Reaching the World.</span>
            </h2>
            <p style={{ color:"rgba(13,27,42,0.5)", fontSize:"15px", margin:0 }}>Pan-India sourcing · APEDA & Spices Board certified · 6 export regions</p>
          </div>
          <div style={{ position:"relative", borderRadius:"20px", overflow:"hidden", boxShadow:"0 20px 60px rgba(13,27,42,0.15)", border:"1px solid rgba(13,27,42,0.1)", background:"#b8d8e8" }}>
            <div style={{ position:"relative" }}>
              <div style={{ background:"#cde8f5", position:"relative" }}><img src="/worldmap.png" alt="World Map" style={{ width:"100%", display:"block", filter:"invert(1) sepia(1) saturate(2) hue-rotate(180deg) brightness(0.7)" }} /></div>
              <div style={{ position:"absolute", inset:0 }}>
                <div style={{ position:"absolute", left:"67%", top:"47%", transform:"translate(-50%,-50%)" }}>
                  ...
                  <div style={{ position:"absolute", top:"16px", left:"50%", transform:"translateX(-50%)", background:"rgba(13,27,42,0.9)", border:"1px solid #C4930A", borderRadius:"6px", padding:"2px 8px", whiteSpace:"nowrap" }}>
                    <span style={{ color:"#C4930A", fontSize:"9px", fontWeight:700 }}>🇮🇳 INDIA</span>
                  </div>
                </div>
                {[[60,44],[58.5,46],[59.5,45],[61,43],[57.5,47]].map(([x,y],i)=>(
                  <div key={i} style={{ position:"absolute", left:`${x}%`, top:`${y}%`, transform:"translate(-50%,-50%)" }}>
                    <div style={{ width:"10px", height:"10px", borderRadius:"50%", background:"#C4930A", border:"1.5px solid white", boxShadow:"0 0 8px rgba(196,147,10,0.7)" }}/>
                  </div>
                ))}
                {[[46,26],[48,26],[47.5,27.5],[49,25],[46.5,24.5]].map(([x,y],i)=>(
                  <div key={i} style={{ position:"absolute", left:`${x}%`, top:`${y}%`, transform:"translate(-50%,-50%)" }}>
                    <div style={{ width:"10px", height:"10px", borderRadius:"50%", background:"#C4930A", border:"1.5px solid white", boxShadow:"0 0 8px rgba(196,147,10,0.7)" }}/>
                  </div>
                ))}
                {[[18,34],[17,27]].map(([x,y],i)=>(
                  <div key={i} style={{ position:"absolute", left:`${x}%`, top:`${y}%`, transform:"translate(-50%,-50%)" }}>
                    <div style={{ width:"10px", height:"10px", borderRadius:"50%", background:"#C4930A", border:"1.5px solid white", boxShadow:"0 0 8px rgba(196,147,10,0.7)" }}/>
                  </div>
                ))}
                <div style={{ position:"absolute", left:"27%", top:"64%", transform:"translate(-50%,-50%)" }}>
                  <div style={{ width:"10px", height:"10px", borderRadius:"50%", background:"#C4930A", border:"1.5px solid white", boxShadow:"0 0 8px rgba(196,147,10,0.7)" }}/>
                </div>
                {[[76,55],[83,32],[82,71]].map(([x,y],i)=>(
                  <div key={i} style={{ position:"absolute", left:`${x}%`, top:`${y}%`, transform:"translate(-50%,-50%)" }}>
                    <div style={{ width:"10px", height:"10px", borderRadius:"50%", background:"#C4930A", border:"1.5px solid white", boxShadow:"0 0 8px rgba(196,147,10,0.7)" }}/>
                  </div>
                ))}
                {[[49,54],[55,57],[52,73],[54,42]].map(([x,y],i)=>(
                  <div key={i} style={{ position:"absolute", left:`${x}%`, top:`${y}%`, transform:"translate(-50%,-50%)" }}>
                    <div style={{ width:"10px", height:"10px", borderRadius:"50%", background:"#C4930A", border:"1.5px solid white", boxShadow:"0 0 8px rgba(196,147,10,0.7)" }}/>
                  </div>
                ))}
                {[[70,45],[67.5,52]].map(([x,y],i)=>(
                  <div key={i} style={{ position:"absolute", left:`${x}%`, top:`${y}%`, transform:"translate(-50%,-50%)" }}>
                    <div style={{ width:"10px", height:"10px", borderRadius:"50%", background:"#C4930A", border:"1.5px solid white", boxShadow:"0 0 8px rgba(196,147,10,0.7)" }}/>
                  </div>
                ))}
            ...
            <div style={{ background:"white", padding:"14px 24px", display:"flex", alignItems:"center", gap:"24px", borderTop:"1px solid rgba(13,27,42,0.08)", flexWrap:"wrap" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                <div style={{ width:"10px", height:"10px", borderRadius:"50%", background:"#C4930A" }}/>
                <span style={{ color:"rgba(13,27,42,0.55)", fontSize:"12px" }}>Export regions</span>
              </div>
              <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                <div style={{ width:"10px", height:"10px", borderRadius:"50%", background:"#C4930A", border:"2px solid white", boxShadow:"0 0 8px rgba(196,147,10,0.8)" }}/>
                <span style={{ color:"rgba(13,27,42,0.55)", fontSize:"12px" }}>India — source country</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1.1fr 0.9fr", gap:"48px", alignItems:"start" }}>
            <div style={{ paddingTop:"16px" }}>
              <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"20px" }}>
                ...
                <span style={{ color:"#C4930A", fontSize:"11px", fontWeight:700, letterSpacing:"0.2em", textTransform:"uppercase" }}>Share Your Experience</span>
              </div>
              <h2 style={{ fontFamily:"DM Serif Display,Georgia,serif", fontSize:"clamp(22px,2.5vw,32px)", color:"#0D1B2A", margin:"0 0 14px", fontWeight:400, lineHeight:1.2 }}>Your feedback shapes every shipment we send.</h2>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"10px" }}>
                {[
                  { text:"Quality was exactly as per COA. Shipment arrived on time with full documentation. Will definitely order again.", name:"Ahmed Al-Rashidi", country:"🇦🇪 UAE", role:"Import Manager", rating:5 },
                  { text:"Best turmeric we have sourced from India. Curcumin content was lab-verified and consistent across batches.", name:"Lars Eriksson", country:"🇸🇪 Sweden", role:"Food Brand Owner", rating:5 },
                ].map((t, i) => (
                  <div key={i} style={{ background:"#F5F0E8", borderRadius:"14px", padding:"16px 20px", border:"1px solid rgba(13,27,42,0.06)" }}>
                    <div style={{ display:"flex", gap:"2px", marginBottom:"10px" }}>
                      {[1,2,3,4,5].map(s => <span key={s} style={{ color: s <= t.rating ? "#E8A020" : "rgba(13,27,42,0.15)", fontSize:"16px" }}>★</span>)}
                    </div>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:"10px", paddingTop:"12px", borderTop:"1px solid rgba(13,27,42,0.06)" }}>
                      <div style={{ width:"36px", height:"36px", borderRadius:"10px", background:"linear-gradient(135deg,#C4930A,#E8A020)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"16px", flexShrink:0 }}>
                        {t.country.split(" ")[0]}
                      </div>
                      <div>
                        <p style={{ fontWeight:700, color:"#0D1B2A", fontSize:"13px", margin:0 }}>{t.name}</p>
                        <p style={{ color:"rgba(13,27,42,0.4)", fontSize:"11px", margin:0 }}>{t.role} · {t.country.split(" ").slice(1).join(" ")}</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div style={{ background:"#F5F0E8", borderRadius:"20px", padding:"28px", border:"1px solid rgba(13,27,42,0.06)" }}>
              <FeedbackSection />
            </div>
          </div>
        </div>
      ...
      {/* CTA */}
      <section style={{ padding:"56px 0", background:"linear-gradient(135deg,#92400e,#C4930A,#E8A020)" }}>
        <div style={{ maxWidth:"1200px", margin:"0 auto", padding:"0 48px" }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"48px", alignItems:"start" }}>
            <div>
              <h2 style={{ fontFamily:"DM Serif Display,Georgia,serif", fontSize:"clamp(24px,3vw,38px)", color:"white", marginBottom:"12px", fontWeight:400, lineHeight:1.2 }}>Ready to Source from India's Best?</h2>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"8px", marginBottom:"16px" }}>
                {[
                  { icon:"✉️", text:"info@silasya.com" },
                  { icon:"📞", text:"+91 7259829005" },
                  { icon:"📍", text:"HSR Layout, Bengaluru – 560102" },
                ].map((item, i) => (
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                    <span style={{ fontSize:"16px" }}>{item.icon}</span>
                    <span style={{ color:"rgba(255,255,255,0.75)", fontSize:"14px" }}>{item.text}</span>
                  </div>
                ))}
              </div>
              <a href="/Shumitra_Export_Catalogue.pdf" download="Shumitra_Export_Catalogue.pdf"
                style={{ display:"inline-flex", alignItems:"center", gap:"8px", background:"rgba(255,255,255,0.12)", border:"1px solid rgba(255,255,255,0.3)", color:"white", fontWeight:600, padding:"12px 20px", borderRadius:"12px", textDecoration:"none", fontSize:"13px", width:"fit-content" }}>
                ⬇ Download Export Catalogue (PDF)
              </a>
            </div>
            <div style={{ background:"white", borderRadius:"20px", padding:"28px", boxShadow:"0 16px 48px rgba(0,0,0,0.2)" }}>
              ...
              <h3 style={{ fontFamily:"DM Serif Display,Georgia,serif", fontSize:"24px", color:"#0D1B2A", margin:"0 0 20px", fontWeight:400, lineHeight:1.2 }}>Get a Quote<br/>in 24 Hours</h3>
              <div style={{ width:"32px", height:"2px", background:"#C4930A", marginBottom:"20px", borderRadius:"2px" }} />
              <CtaForm />
            </div>
          </div>
        </div>
